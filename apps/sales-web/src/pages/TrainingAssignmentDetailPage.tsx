...
export function TrainingAssignmentDetailPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 pb-8">
    ...
    <PageHeader eyebrow="Assignment detail" title={assignment.program.name} description={`Phân công cho ${assignment.assignedTo.displayName}. Chỉ hiển thị metadata tiến độ, không hiển thị hội thoại riêng tư.`} action={<Badge className={assignmentStateClass(assignment.state)}>{assignmentStateLabel(assignment.state)}</Badge>} />
    ...
    <div className="grid gap-4 lg:grid-cols-[1fr_1.4fr]">
      <Card className="p-5 sm:p-6"><h2 className="text-base font-bold text-ink">Thông tin phân công</h2><dl className="mt-4 grid gap-4 text-sm"><Info label="Nhân viên" value={`${assignment.assignedTo.displayName} · ${assignment.assignedTo.email}`} /><Info label="Người giao" value={assignment.assignedBy.displayName} /><Info label="Ngày giao" value={formatAssignmentDate(assignment.assignedAt)} /><Info label="Hạn hoàn thành" value={formatAssignmentDate(assignment.dueAt)} warning={assignment.isOverdue} /></dl><div className="mt-5 border-t border-border pt-5"><AssignmentProgress completed={assignment.completedItems} total={assignment.totalItems} percent={assignment.progressPercent} /></div></Card>
      <Card className="p-5 sm:p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-bold text-ink">Nội dung chương trình</h2><p className="mt-1 text-xs text-ink-muted">Trạng thái được suy ra từ các phiên của nhân viên.</p></div><Button aria-label="Làm mới tiến độ" size="sm" variant="ghost" onClick={() => void load()}><RefreshCw className="h-4 w-4" /></Button></div><div className="mt-4 space-y-3">{assignment.items.map((item) => <div key={item.id} className="rounded-xl border border-border bg-surface-subtle/35 p-4"><div className="flex items-start justify-between gap-3"><p className="text-sm font-bold text-ink">{item.sortOrder}. {item.personaLabel ?? item.personaId}</p><span className="text-xs font-semibold text-brand">{assignmentItemStateLabel(item.state)}</span></div><p className="mt-1 text-xs text-ink-secondary">{item.scenarioLabel ?? item.scenarioId}</p><p className="mt-2 text-xs text-ink-muted">{trainingModeLabel(item.mode)}</p></div>)}</div></Card>
    </div>
