...
export function ScenarioManagementPage() {
  ...
  return <div className="space-y-6"><PageHeader eyebrow="Training content" title="Quản lý tình huống" description="Tình huống độc lập, có thể liên kết với nhiều Persona và được ghim theo phiên bản trong chương trình." action={<Link to="/manage/scenarios/new"><Button icon={<Plus className="h-4 w-4" />}>Tạo tình huống</Button></Link>} />
    <div className="grid gap-4 lg:grid-cols-2">{items.map((item) => <Link key={item.id} to={`/manage/scenarios/${encodeURIComponent(item.id)}`}><Card className="h-full p-5 hover:border-brand"><div className="flex items-start justify-between gap-3"><div><h2 className="font-bold text-ink">{item.title}</h2><p className="mt-1 text-xs text-ink-muted">{item.id}</p></div><Badge>{item.archivedAt ? 'ARCHIVED' : item.draft ? `DRAFT v${item.draft.version}` : `PUBLISHED v${item.latestPublished?.version ?? '-'}`}</Badge></div><p className="mt-4 text-sm text-ink-secondary">Liên kết {item.linkedPersonaCount} Persona</p></Card></Link>)}</div>
  </div>
}
...
export function ScenarioEditorPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><PageHeader eyebrow="Scenario" title={isNew ? 'Tạo tình huống' : detail?.title ?? 'Tình huống'} description="Giao diện chỉ quản lý các trường nội dung an toàn; cấu hình thực thi nội bộ được hệ thống tạo tự động." action={detail && <Badge>{detail.archivedAt ? 'ARCHIVED' : `${detail.currentVersion.status} · v${detail.currentVersion.version}`}</Badge>} />
