...
export class DatabaseSessionRepository implements SessionRepository {
  ...
  async findById(id: string): Promise<SimulationSession | null> {
    ...
    return {
      ...
      scenarioVersionId: stored.scenarioVersionId,
      contentSnapshot: stored.contentSnapshot as unknown as SimulationSession["contentSnapshot"],
      mode: stored.mode,
  ...
  async save(session: SimulationSession): Promise<void> {
    const sessionData = {
      ...
      personaVersionId: session.personaVersionId ?? null,
      scenarioVersionId: session.scenarioVersionId ?? null,
      contentSnapshot: nullableJson(session.contentSnapshot),
